  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-slate-800">Reports & Logs</h1>
          <p className="text-slate-500 text-xs mt-0.5 font-medium">Audit employee learning cycles, course progress percentages, and final test results.</p>
        </div>
        <button
          onClick={fetchReportData}
          className="flex items-center space-x-1.5 bg-white hover:bg-slate-50 text-slate-600 border border-slate-205 text-xs font-semibold px-3 py-2 rounded-xl transition shadow-sm"
        >
      ...
      {/* Report Tabs */}
      <div className="border-b border-slate-205 flex space-x-6">
        <button
          onClick={() => {
      ...
      {/* Filter Control Box */}
      <div className="bg-white border border-slate-100 p-4 rounded-2xl flex flex-col md:flex-row gap-3.5 shadow-sm shadow-slate-100/40">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm shadow-slate-100/40 overflow-hidden">
        {activeReportTab === 'progress' ? (
          /* =======================================
             PROGRESS REPORT GRID
             ======================================= */
          filteredProgress.length === 0 ? (
            <p className="text-center py-12 text-slate-400 text-xs font-medium">No progress logs match your query.</p>
          ) : (
              ...
              <table className="w-full text-left text-xs text-slate-650">
                <thead>
                  <tr className="text-[10px] uppercase tracking-widest text-slate-400 border-b border-slate-100">
                    ...
                    <th className="pb-3 font-extrabold">Completed Modules</th>
                    <th className="pb-3 font-extrabold">Completion Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100/60 font-medium">
                  {filteredProgress.map((p) => (
                    <tr key={p.assignmentId} className="hover:bg-slate-50/50 transition duration-75">
                      <td className="py-4">
                        <div className="font-bold text-slate-700">{p.employeeName}</div>
                        <div className="text-[10px] text-slate-450 leading-tight font-semibold mt-0.5">{p.employeeEmail}</div>
                      </td>
                      <td className="py-4 font-semibold text-slate-500">{p.department}</td>
                      <td className="py-4 font-bold text-slate-700">{p.courseTitle}</td>
                      <td className="py-4">
                        <div className="flex items-center space-x-2 w-36">
                          <span className="text-[11px] font-bold text-slate-700 shrink-0">{p.percentage}%</span>
                      ...
                      <td className="py-4">
                        ...
                        {p.percentage === 100 && p.status === 'in-progress' && (
                          <div className="text-[9px] text-amber-600 font-bold mt-1.5">
          ...
          filteredQuizzes.length === 0 ? (
            <p className="text-center py-12 text-slate-400 text-xs font-medium">No exam records found.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-650">
                <thead>
                  <tr className="text-[10px] uppercase tracking-widest text-slate-400 border-b border-slate-100">
                    <th className="pb-3 font-extrabold">Employee</th>
                    <th className="pb-3 font-extrabold">Course Title</th>
                    <th className="pb-3 font-extrabold">Quiz Title</th>
                    <th className="pb-3 font-extrabold">Score Scored</th>
                    <th className="pb-3 font-extrabold">Exams Date</th>
                    <th className="pb-3 font-extrabold text-right">Result</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100/60 font-medium">
                  {filteredQuizzes.map((q) => (
                    <tr key={q.attemptId} className="hover:bg-slate-50/50 transition duration-75">
                      <td className="py-4">
                        <div className="font-bold text-slate-700">{q.employeeName}</div>
                        <div className="text-[10px] text-slate-450 leading-tight font-semibold mt-0.5">{q.employeeEmail}</div>
                      </td>
                      <td className="py-4 font-semibold text-slate-750">{q.courseTitle}</td>
                      <td className="py-4 text-xs font-semibold text-slate-500">{q.quizTitle}</td>
                      <td className="py-4">
          ...
          filteredCodingTasks.length === 0 ? (
            <p className="text-center py-12 text-slate-400 text-xs font-medium">No coding tasks found.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-650">
                <thead>
                  <tr className="text-[10px] uppercase tracking-widest text-slate-400 border-b border-slate-100">
                    <th className="pb-3 font-extrabold">Employee</th>
                    <th className="pb-3 font-extrabold">Course Title</th>
                    <th className="pb-3 font-extrabold">GitHub Link</th>
                    <th className="pb-3 font-extrabold">Status & Feedback</th>
                    <th className="pb-3 font-extrabold text-right">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100/60 font-medium">
                  {filteredCodingTasks.map((ct) => (
                    <tr key={ct.progressId} className="hover:bg-slate-50/50 transition duration-75">
                      <td className="py-4 align-top">
                        <div className="font-bold text-slate-700">{ct.employeeName}</div>
                        <div className="text-[10px] text-slate-450 leading-tight font-semibold mt-0.5">{ct.employeeEmail}</div>
                      </td>
                      <td className="py-4 font-semibold text-slate-750 align-top">{ct.courseTitle}</td>
                      <td className="py-4 align-top">
                        <a href={ct.githubLink} target="_blank" rel="noreferrer" className="text-indigo-600 hover:text-indigo-800 underline break-all font-semibold">
                          {ct.githubLink}
                        </a>
                        {ct.employeeMessage && (
                          <div className="mt-2 text-[10px] text-slate-500 bg-slate-50 p-2 rounded-lg border border-slate-100 border-l-2 border-l-indigo-300 italic">
                      ...
                      <td className="py-4 align-top text-right min-w-[160px]">
                        <div className="flex flex-col space-y-2 items-end">
                          <select
                            className="text-xs border border-slate-200 rounded-md p-1 focus:outline-none"
                            onChange={(e) => {
